...
async def list_restores(
    ...
    query: Annotated[
        ...
        Query(
            description=(
                "Filter restores using format 'field|value' or "
                "'field|operator|value'. "
                "Operators: eq, ne, gt, gte, lt, lte, in, nin. "
                f"Valid fields: {', '.join([f.value for f in RestoreQueryField])}"
            ),
            openapi_examples={
                "by_backup_id": {
    ...
    order: Annotated[
        ...
        Query(
            description=(
                ...
                f"Valid fields: {', '.join([f.value for f in RestoreOrderField])}. "
    ...
    valid_query_fields = {f.value for f in RestoreQueryField}
    ...
    valid_order_fields = {f.value for f in RestoreOrderField}
