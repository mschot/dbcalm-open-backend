...
class Mariadb(adapter.Adapter):    
    ...
    def full_backup(self, identifier: str) -> Process:                
        ...
        return self.command_runner.execute(command, 'backup', {'identifier': identifier})

    def incremental_backup(self, identifier: str, from_identifier: str) -> Process:
        command = self.command_builder.build_incremental_backup_cmd(identifier, from_identifier)
        return self.command_runner.execute(command, 'backup', {'identifier': identifier, 'from_identifier': from_identifier})
