...
cp "templates/$PACKAGE_NAME-cmd-server.service" "build/debian/usr/lib/systemd/system/"
...
pyinstaller --onefile --hidden-import passlib.handlers.bcrypt --clean --workpath=./build/pyinstaller --distpath=dist/production ${PACKAGE_NAME}-api.py
pyinstaller --onefile --clean --workpath=./build/pyinstaller --distpath=dist/production ${PACKAGE_NAME}-cmd-server.py
rm -f ${PACKAGE_NAME}-api.spec
rm -f ${PACKAGE_NAME}-cmd-server.spec
...
cp "dist/production/${PACKAGE_NAME}-api" "build/debian/usr/bin/"
cp "dist/production/${PACKAGE_NAME}-cmd-server" "build/debian/usr/bin/"
